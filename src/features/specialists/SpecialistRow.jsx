import styled from "styled-components";

import { useDeleteSpecialist } from "./useDeleteSpecialist";
import { formatCurrency } from "../../utils/helpers";
import { HiPencil, HiTrash } from "react-icons/hi2";
// import { useCreateSpecialist } from "./useCreateSpecialist";
import Modal from "../../ui/Modal";
import ConfirmDelete from "../../ui/ConfirmDelete";
import Table from "../../ui/Table";
import Menus from "../../ui/Menus";
import CreateSpecialistForm from "./CreateSpecialistForm";

// const TableRow = styled.div`
//   display: grid;
//   grid-template-columns: 0.6fr 1.8fr 2.2fr 1fr 1fr 1fr;
//   column-gap: 2.4rem;
//   align-items: center;
//   padding: 1.4rem 2.4rem;

//   &:not(:last-child) {
//     border-bottom: 1px solid var(--color-grey-100);
//   }
// `;

const Img = styled.img`
    display: block;
    width: 6.4rem;
    aspect-ratio: 3 / 2;
    object-fit: cover;
    object-position: center;
    transform: scale(1.5) translateX(-7px);
`;

const Specialist = styled.div`
    font-size: 1.6rem;
    font-weight: 600;
    color: var(--color-grey-600);
    font-family: "Sono";
    text-transform: capitalize;
`;

const Price = styled.div`
    font-family: "Sono";
    font-weight: 600;
`;

const Location = styled.div`
    font-family: "Sono";
    font-weight: 600;
    text-transform: capitalize;
`;

function SpecialistRow({ specialist }) {
    const { isDeleting, deleteSpecialist } = useDeleteSpecialist();
    // const { isCreating, createSpecialist } = useCreateSpecialist();

    const {
        id: specialistId,
        fullName,
        specialization,
        location,
        imageUrl,
        price,
    } = specialist;

    // function handleDuplicate() {
    //     createSpecialist({
    //         name: `Copy of ${name}`,
    //         maxCapacity,
    //         regularPrice,
    //         discount,
    //         image,
    //         description,
    //     });
    // }

    return (
        <Table.Row>
            <Img src={imageUrl} />
            <Specialist>{fullName}</Specialist>
            <Specialist>{specialization}</Specialist>
            <Price>{formatCurrency(price)}</Price>
            <Location>{location}</Location>

            <div>
                <Modal>
                    <Menus.Menu>
                        <Menus.Toggle id={specialistId} />

                        <Menus.List id={specialistId}>
                            {/* <Menus.Button
                                icon={<HiSquare2Stack />}
                                // onClick={handleDuplicate}
                                onClick={() => console.log("hey")}
                                disabled={isCreating}>
                                Duplicate
                            </Menus.Button> */}

                            <Modal.Open opens='edit'>
                                <Menus.Button icon={<HiPencil />}>
                                    Edit
                                </Menus.Button>
                            </Modal.Open>

                            <Modal.Open opens='delete'>
                                <Menus.Button icon={<HiTrash />}>
                                    Delete
                                </Menus.Button>
                            </Modal.Open>
                        </Menus.List>

                        <Modal.Window name='edit'>
                            <CreateSpecialistForm
                                specialistToEdit={specialist}
                            />
                        </Modal.Window>

                        <Modal.Window name='delete'>
                            <ConfirmDelete
                                resourceName='specialist'
                                disabled={isDeleting}
                                onConfirm={() => deleteSpecialist(specialistId)}
                            />
                        </Modal.Window>
                    </Menus.Menu>
                </Modal>
            </div>
        </Table.Row>
    );
}

export default SpecialistRow;
